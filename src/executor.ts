...
export async function executePlan(config: Config): Promise<string> {
  if (!config.databaseUrl) {
    throw new Error("DATABASE_URL is required for execution phase. Provide during setup or add to .env file.");
  }
...
function extractQueriesFromPlan(planContent: string): Array<{ label: string; type: "sql" | "hogql"; query: string }> {
  ...
  // Group 3: Content
  const blockRegex = /(?:(?:###?\s+(?:Query\s+\d+:?\s*)?(.+?))\s*\n)?```(sql|hogql)\n([\s\S]+?)\n```/gi;
...
export async function checkInterimResults(config: Config): Promise<string | null> {
  const resultsPath = path.join(config.outputDir, INTERIM_RESULTS_FILE);
