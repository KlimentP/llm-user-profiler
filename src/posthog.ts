...
export async function executePostHogQuery(config: Config, query: string): Promise<any[]> {
  if (!config.posthogApiKey || !config.posthogProjectId) {
    throw new Error("PostHog API Key and Project ID are required for PostHog queries.");
  }
