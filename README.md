...
-   **Execution Modes**:
    -   **Full**: Runs queries as-is.
    -   **Incremental**: Replaces `{{since_last_run}}` / `{{until_now}}` placeholders with timestamps from the last successful execution.
-   **Execution**:
    -   SQL queries are run against the configured PostgreSQL database.
    -   HogQL queries are sent to the PostHog API.
-   **Data Aggregation**: Results from all queries are aggregated and saved to a timestamped file in `interim_results/`. This serves as the raw data layer for the profiling phase.
-   **Safety**: SQL execution is restricted to read-only statements (`SELECT`/`WITH`) and runs in a read-only transaction.

### 3. Profiling Phase (`src/profiler.ts`)
The final phase synthesizes the data into human-readable profiles.
-   **Synthesis**: The LLM is fed the `analysis_plan.md` (for context/definitions) and the latest `interim_results/*.json` file (the data).
...
-   **Quality Layer**: The output is validated and enriched with profile-level confidence scores plus run-level validation metadata.
...
-   `interim_results/interim_results_<timestamp>.json`: The raw data fetched from your data sources.
-   `user_profiles.json`: The final, detailed user profiles in JSON format.
-   `run_manifest.json`: Tracks the latest plan/interim/profile artifacts for reliable resume behavior.
